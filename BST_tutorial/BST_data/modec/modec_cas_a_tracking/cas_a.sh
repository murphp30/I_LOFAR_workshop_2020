...
if [ $bits -eq 8 ]; then
	...
	rspctl --bitmode=8
	sleep 10 
	beamctl --antennaset=$antennaset --rcus=0:191 --rcumode=$rcumode --band=$band --subbands=$subbands --beamlets=$beamlets --anadir=$CAS_A --digdir=$CAS_A > $datapath/beamctl.log 2>&1 &
else
	rspctl --bitmode=16
	beamctl --antennaset=$antennaset --rcus=0:191 --rcumode=$rcumode --band=$band --subbands=$subbands --beamlets=$beamlets --anadir=$CAS_A --digdir=$CAS_A > $datapath/beamctl.log 2>&1 &
fi
